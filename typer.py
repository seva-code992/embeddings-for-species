...
all_ID = potra_ID + picab_ID + pinsy_ID + arabidopsis_ID + bepen_ID + tieton_ID
all_description = picab_description + pinsy_description + potra_description + arabidopsis_description + tieton_description + bepen_description
...
all_embeddings = potra_embeddings + picab_embeddings  + pinsy_embeddings + bepen_embeddings  + tieton_embeddings + arabidopsis_embeddings 
all_ids = picab_ids + pinsy_ids + tieton_ids + bepen_ids + potra_ids + arabidopsis_ids
...
@app.command()
def search(query: str, number:int, pinsy: bool = False, picab: bool = False, potra: bool = False, arabidopsis: bool = False, bepen: bool = False, tieton: bool = False):
    ...
    if pinsy: 
        ...
        query= input("You are searching for genes within all available species within our database. What kind of genes are you looking for?")
        number = int(input("How many results do you want to see? "))
        query_embedding = model.encode_query(query)
        similarities = model.similarity(query_embedding, all_embeddings).squeeze()
        indices= similarities.argsort(descending=True).squeeze().tolist()
        print(f"\nTop {number} results for \"{query}\": ")
        for i in range(number):
            index = indices[i]
            score = similarities[index].item()
            print(f"\nGene: {all_ids[index]} \nIndices: {all_embeddings[index]} \nSimilarity score: {score} \nDescription: {all_description[index]}")
